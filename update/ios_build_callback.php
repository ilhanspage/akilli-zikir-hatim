...
$expected = setting('ios_build_callback_token', '');
$given = $_GET['token'] ?? '';
...
$pdo = db();
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS ios_build_requests (
...
$stmt = $pdo->prepare('UPDATE ios_build_requests SET status=?, artifact_url=?, response_text=?, updated_at=NOW() WHERE request_uid=?');
